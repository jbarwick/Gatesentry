...
- Binary version is `GATESENTRY_VERSION` in `main.go` (current: `2.0.0-beta.6`). That string is the only release tag. Do not invent a different version for the image.
...
- Production image on monster-jj is `2.0.0-beta.6`. Next ship: bump `GATESENTRY_VERSION` in `main.go`, then `./build.sh && ./release.sh && ./deploy.sh`.
